...
class MarkdownExporterApp:
    def __init__(self, root):
        ...
        #补充路径输入
        add_frame = ttk.LabelFrame(export_frame, text="4. 补充Markdown导出路径", padding=10)
        add_frame.pack(fill=tk.X, padx=5, pady=5)
        
        self.add_path = tk.StringVar()
        ttk.Entry(add_frame, textvariable=self.add_path, state='writable', width=70).pack(
            side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
    ...
    def export_markdown(self):
        ...
        try:
            ...
            # 创建资源文件夹
            asset_folder = Path(self.asset_path.get() +'/'+self.add_path.get().strip()+ f"{file_name}_localdata")
            ...
            for old_link, new_link in link_map.items():
                ...
                new_link=self.add_path.get().strip()+new_link
                ...
                content=content.replace("src=\""+escaped_old_link+"\"",new_link)
